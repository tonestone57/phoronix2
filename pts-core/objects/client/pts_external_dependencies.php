	...
	public static function install_dependencies(&$test_profiles, $no_prompts = false, $skip_tests_with_missing_dependencies = false, $report_progress = false)
	{
		...
		if(count($required_external_dependencies) > 0 && count($dependencies_to_install) == 0 && $skip_warning_on_unmet_deps == false)
		{
			...
			if(count($to_report) > 0)
			{
				...
				echo pts_user_io::display_text_list($to_report);
			...
			for($i = 0; $i < count($file) && $file_is_there == false; $i++)
			{
				...
				else if(isset($file[$i][1]) && $file[$i][0] != '/')
				{
					...
					if(substr($file[$i], -2) == '.h' || substr($file[$i], -4) == '.hpp')
					{
						...
						$possible_paths = array_merge(array('/usr/local/include/', '/usr/target/include/', '/usr/include/', '/boot/system/develop/headers/', '/opt/homebrew/include/', '/opt/homebrew/opt', '/opt/homebrew/'), pts_file_io::glob('/usr/include/*-linux-gnu/'));
					...
					else if(strpos($file[$i], '.so') !== false || substr($file[$i], -2) == '.a')
					{
						...
						$possible_paths = array_merge(array('/usr/local/lib/', '/usr/lib/', '/usr/lib64/', '/boot/system/lib/', '/boot/system/develop/lib/', '/usr/lib/arm-linux-gnueabihf/',  '/opt/homebrew/include/', '/opt/homebrew/opt', '/opt/homebrew/'), pts_file_io::glob('/usr/lib/*-linux-gnu/'));
